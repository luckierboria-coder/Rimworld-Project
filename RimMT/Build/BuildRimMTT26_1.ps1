...
if($boot -match 'SingleCallCandidatePartition\.Apply'){ throw 'candidate partition still installed' }
...
$removeMarker='<Compile Remove="AI\SingleCallCandidatePartition.cs" />'
if(-not $csproj.Contains($removeMarker)){ throw 'candidate partition is not excluded from compilation' }

$s4=Get-Content (Join-Path $root 'RimMT/Source/RimMT/AI/JobGiverSlowSearch0419S.cs') -Raw
foreach($marker in @('TargetedPrefilterKind.FightFires','RimWorld.WorkGiver_FightFires','targetedFightFiresRejected','worker.Map.areaManager.Home[fire.Position]','IsTargetedPrefilterAuthoritySafe')){
