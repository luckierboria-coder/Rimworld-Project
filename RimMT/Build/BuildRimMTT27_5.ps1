...
'ApplyRimMTV093T27ParallelWorkKernel.ps1','ApplyRimMTV093T27_1WorkPlanWindow.ps1','ApplyRimMTV093T27_2SafetyLayerReset.ps1','ApplyRimMTV093T27_3WaitStallTrace.ps1','ApplyRimMTV093T27_4DiagnosticsSplit.ps1','ApplyRimMTV093T27_5StutterFoundation.ps1'
)
...
# ---- T27.5 production assertions ----
$boot=Get-Content (Join-Path $root 'RimMT/Source/RimMT/Bootstrap/RimMTBootstrap.cs') -Raw
if($boot -notmatch '0\.9\.3-t27\.5-stutter-foundation'){ throw 'T27.5 version marker missing' }
...
if($boot -match 'WaitStallPatches093T27_3\.Apply\(harmony\)'){ throw 'Wait-stall diagnostics returned to RimMT.dll' }
...
foreach($required in @('CaptureMapQuarantineFrames = 3600','CaptureDisabledUntilFrame','profileCapturePressureBypass','profileCaptureMapQuarantines','maxCapturePairUs','AdaptiveLoadBalancer.Pressure == LoadPressure.Critical')){
  if(-not $reach.Contains($required)){ throw "T27.5 Reach hardening marker missing: $required" }
}

$merge=Get-Content (Join-Path $root 'RimMT/Source/RimMT/AI/WorkGiverMergePartnerIndex093T4.cs') -Raw
foreach($required in @('ThingCanStackAuthorityMode','net.avilmask.rimworld.mod.CommonSense','CommonSense.CompIngredients_CanStackWith_CommonSensePatch','t.def.IsIngestible','commonSenseIngestibleBypass')){
  if(-not $merge.Contains($required)){ throw "T27.5 CommonSense HaulMerge marker missing: $required" }
}
# The compatibility is deliberately narrow: ingestible stacks must still bypass T4.
if($merge -notmatch 'if \(t != null && t\.def != null && t\.def\.IsIngestible\)'){ throw 'CommonSense ingestible fail-open guard missing' }

$bill=Get-Content (Join-Path $root 'RimMT/Source/RimMT/AI/PersistentDoBillIndex092.cs') -Raw
foreach($required in @('PackageReadiness','packageReadinessBuilds','packageReadinessReuses','CurrentScopeStartTicks')){
  if(-not $bill.Contains($required)){ throw "T27.5 DoBill package reuse marker missing: $required" }
}
foreach($forbidden in @('TicksGame +','LastReadyTick','cross-package readiness','Task.Run(','SpinWait','Thread.Sleep(')){
  if($bill -match [regex]::Escape($forbidden)){ throw "DoBill readiness escaped package-local policy: $forbidden" }
}
...
if($diagAbout -notmatch 'RimMT Diagnostics v0\.2'){ throw 'Diagnostics v0.2 metadata missing' }
...
foreach($required in @('0.2.0','DiagnosticsV02','LongestCurrentWait','PatherChildren','WorkGiverSampled','ReachCaptureRegionAllows','PauseResume','VFEEmpire.WorldComponent_Hierarchy')){
...
$mainZip=Join-Path $build 'RimMT_V0.9.3_T27.5_Production.zip'; $diagZip=Join-Path $build 'RimMT_Diagnostics_v0.2.zip'; $bundleZip=Join-Path $build 'RimMT_T27.5_With_Diagnostics_Bundle.zip'
