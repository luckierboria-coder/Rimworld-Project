...
'ApplyRimMTV093T27ParallelWorkKernel.ps1','ApplyRimMTV093T27_1WorkPlanWindow.ps1'
)
foreach($s in $steps){
  Write-Host "== $s =="
...
if($boot -notmatch '0\.9\.3-t27\.1-work-plan-window'){ throw 'T27.1 version marker missing' }
if($boot -notmatch 'ParallelWorkKernel093T27\.Apply'){ throw 'T27.1 kernel is not installed' }
if($boot -match 'SingleCallCandidatePartition\.Apply'){ throw 'T26 low-ROI same-call partition returned' }
if($boot -match 'AsyncJobCandidatePlan04182\.Apply' -or $boot -match 'JobDecisionRootAttribution093T25\.Apply'){ throw 'T25 next-call/cache experiment leaked into T27.1' }
...
foreach($required in @(
  ...
  if(-not $kernel.Contains($required)){ throw "T27.1 kernel marker missing: $required" }
}

# Historical regression guards: T27 package-wide scanner tax, T26 waiting, T25 cross-call result reuse.
foreach($forbidden in @(
  'WorkGiversInOrderNormal','WorkGiversInOrderEmergency','ClassifySnapshotSource',
  'SpinOnce(','new SpinWait(','.Wait(','.Join(','Thread.Sleep(','ManualResetEvent',
  'AsyncJobCandidatePlan04182')){
  if($kernel -match [regex]::Escape($forbidden)){ throw "T27.1 forbidden regression found: $forbidden" }
}

# Unknown IEnumerable must never be consumed just to opt into parallelism.
foreach($forbidden in @('customGlobalSearchSet.ToList','foreach (Thing t in customGlobalSearchSet)','foreach (Thing thing in customGlobalSearchSet)','new List<Thing>(customGlobalSearchSet)')){
  if($kernel -match [regex]::Escape($forbidden)){ throw "T27.1 dynamic IEnumerable materialization regression: $forbidden" }
}

# Package-local result lifetime: source-use history may persist, DistancePlan may not live in SourceState/HotMapState/static caches.
$ssA=$kernel.IndexOf('private sealed class SourceState')
$ssB=$kernel.IndexOf('private sealed class PlanSlot',$ssA)
if($ssA -lt 0 -or $ssB -lt 0){ throw 'Cannot isolate T27.1 persistent source state' }
$persistentState=$kernel.Substring($ssA,$ssB-$ssA)
if($persistentState -match 'DistancePlan'){ throw 'T27.1 cross-package DistancePlan cache detected in persistent source state' }
if($kernel -match 'static\s+[^;\r\n]*DistancePlan'){ throw 'T27.1 static DistancePlan cache detected' }

# Worker delegate may only build from immutable PersistentMapSearchFabric snapshot data.
$workerStart=$kernel.IndexOf('bool accepted = scheduler.TryEnqueue')
$workerEnd=$kernel.IndexOf('if (accepted)',$workerStart)
if($workerStart -lt 0 -or $workerEnd -lt 0){ throw 'Cannot isolate T27.1 worker delegate' }
...
foreach($unsafe in @('.Position','.Map','.Spawned','CanReach','HasJobOn','JobOnThing','Reserve(','Reservation','Pawn ','UnityEngine')){
  if($worker -match [regex]::Escape($unsafe)){ throw "T27.1 worker dereferences unsafe gameplay state: $unsafe" }
}

# T26.1 zero-wait must remain hard-off.
...
foreach($forbidden in @('SpinOnce(','new SpinWait(','.Wait(','.Join(','Thread.Sleep(','ManualResetEvent')){
  if($epochKernel -match [regex]::Escape($forbidden)){ throw "T27.1 inherited zero-wait violation: $forbidden" }
}

$fabric=Get-Content (Join-Path $root 'RimMT/Source/RimMT/AI/PersistentMapSearchFabric.cs') -Raw
if($fabric -notmatch 'internal static partial class PersistentMapSearchFabric'){ throw 'PersistentMapSearchFabric is not partial' }
if($fabric -notmatch 'internal sealed partial class SourceSnapshot'){ throw 'SourceSnapshot is not partial' }
...
if($pa -lt 0 -or $pb -lt 0){ throw 'Cannot isolate distance-plan worker code' }
$planWorker=$plan.Substring($pa,$pb-$pa)
foreach($unsafe in @('.Position','.Map','.Spawned','CanReach','HasJobOn','JobOnThing','Reserve(','Reservation','Pawn ','UnityEngine')){
  if($planWorker -match [regex]::Escape($unsafe)){ throw "T27.1 distance planner dereferences unsafe gameplay state: $unsafe" }
}
...
$zip=Join-Path $build 'RimMT_V0.9.3_T27.1_Work_Plan_Window.zip'
