...
{
    /// <summary>
    /// JS1.1 S5.2 diagnostic-only NoResult origin profiler.
    /// First runtime-report click arms a bounded profiling campaign; later report clicks print
    /// the hottest repeated negative WorkGiver origins. Gameplay authority is unchanged.
    /// </summary>
    [StaticConstructorOnStartup]
    internal static class NoResultOriginProfilerS52
    {
        private const string HarmonyId = "allen.rimmt.s52.noresult";
        ...
        static NoResultOriginProfilerS52()
        {
            try
            {
                ...
                {
                    Log.Warning("[RimMT] S5.2 NoResult Origin Profiler unavailable: runtime report hook target was not found.");
                    return;
                }
                ...
                Log.Message("[RimMT] JS1.1S5.2 NoResult Origin Profiler installed in diagnostic-only mode. First runtime report arms profiling; the next report prints repeated-negative origins. Gameplay search authority is unchanged.");
            }
            catch (Exception ex)
            {
                Log.Warning("[RimMT] S5.2 NoResult Origin Profiler install failed: " + ex.GetType().Name + ": " + ex.Message);
            }
        }

        public static void RuntimeReportPrefix()
        {
            if (!RimMTThreadGuard.IsMainThread || !reportHookInstalled)
                return;
        ...
        public static void RuntimeReportPostfix()
        {
            ...
            lock (Sync)
            {
                if (!campaignActive)
                {
                    Log.Message("[RimMT] S5.2 NoResult Origin Profiler could not arm because no compatible WorkGiver methods were patched. patchFailures=" + patchFailures + ".");
                    return;
                }

                // If profiling was just armed by this very report, there is no useful sample yet.
                int nowTick = CurrentTick();
                if (totalCalls == 0 || nowTick == campaignStartedAtTick)
                {
                    Log.Message("[RimMT] S5.2 NoResult Origin Profiler ARMED: patchedMethods=" + PatchedMethods.Count + ", patchFailures=" + patchFailures + ". Play normally for a while, then run the runtime report again to print the high-frequency repeated NoResult origins.");
                    return;
                }

                Log.Message(BuildSummary(DefaultTopN));
            }
        }
        ...
        private static void EnsureWorkGiverPatches()
        {
            ...
            MethodInfo postfix = AccessTools.Method(typeof(NoResultOriginProfilerS52), nameof(WorkGiverPostfix));
            ...
            for (int i = 0; i < allTypes.Count; i++)
            {
                ...
                for (int m = 0; m < methods.Length; m++)
                {
                    ...
                    catch (Exception ex)
                    {
                        patchFailures++;
                        if (patchFailures <= 8)
                            Log.Warning("[RimMT] S5.2 skipped WorkGiver method " + method + ": " + ex.GetType().Name + ": " + ex.Message);
                    }
                }
            }
        }
        ...
        {
            if (!campaignActive || !RimMTThreadGuard.IsMainThread)
            {
                __state = 0L;
                return;
            }
        ...
        public static void WorkGiverPostfix(WorkGiver __instance, MethodBase __originalMethod, object __result, object[] __args, long __state)
        {
            if (__state == 0L || !campaignActive || !RimMTThreadGuard.IsMainThread || __instance == null || __originalMethod == null)
                return;

            Pawn pawn = null;
            if (__args != null && __args.Length > 0)
                pawn = __args[0] as Pawn;

            long elapsed = Stopwatch.GetTimestamp() - __state;
            bool negative = IsNegative(__originalMethod, __result);
            Record(__instance, __originalMethod, pawn, negative, elapsed);
        }

        private static bool IsNegative(MethodBase method, object result)
        {
            string name = method.Name;
            if (name == "ShouldSkip")
                return result is bool && (bool)result;
            if (name == "HasJobOnThing" || name == "HasJobOnCell")
                return !(result is bool) || !(bool)result;
            if (name == "NonScanJob" || name == "JobOnThing" || name == "JobOnCell")
                return result == null;
            return false;
        }
        ...
        private static void Record(WorkGiver giver, MethodBase method, Pawn pawn, bool negative, long elapsed)
        {
            ...
            stat.RepeatIntervalTickSum += delta == int.MaxValue ? 0 : delta;
            stat.RepeatIntervalSamples += delta == int.MaxValue ? 0 : 1;
        ...
        private static void ResetRepeatState(OriginKey origin, Pawn pawn)
        {
            PawnOriginKey key = new PawnOriginKey(origin, pawn == null ? 0 : pawn.thingIDNumber);
            RepeatStates.Remove(key);
        }
        ...
        private static string BuildSummary(int topN)
        {
            ...
            int nowTick = CurrentTick();
            int elapsedGameTicks = nowTick - campaignStartedAtTick;
            ...
            sb.Append("[RimMT] S5.2 NoResult Origin report: active=").Append(campaignActive)
            ...
            for (int i = 0; i < topN; i++)
            {
                ...
                sb.Append("\n  #").Append(i + 1).Append(' ')
                    ...
                    .Append(", noResult=").Append(stat.Negative)
                    .Append(" (").Append(noResultRatio.ToString("F1")).Append("%)")
                    .Append(", repeated=").Append(stat.RepeatedNegative)
                    .Append(" (").Append(originRepeatRatio.ToString("F1")).Append("%)")
            ...
            if (entries.Count > 0)
            {
                ...
                sb.Append("\n  dominantRepeatedNegativeSource=")
                    ...
                    .Append(". This is a profiling signal only; S5.2 does not cache or suppress any search.");
            }
        ...
        private struct OriginKey : IEquatable<OriginKey>
        {
            ...
            {
                return obj is OriginKey && Equals((OriginKey)obj);
            }
        ...
        private struct PawnOriginKey : IEquatable<PawnOriginKey>
        {
            ...
            internal PawnOriginKey(OriginKey origin, int pawnId)
            {
                Origin = origin;
                PawnId = pawnId;
            }
