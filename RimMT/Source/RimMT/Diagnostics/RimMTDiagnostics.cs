...
namespace RimMT
{
    public static class RimMTDiagnostics
    {
        internal static void LogStartupReport()
        {
            ...
            sb.AppendLine("Reach NO cache: hits=" + ReachabilityNoCache.Hits + ", stores=" + ReachabilityNoCache.Stores);
        ...
        public static void RunWorkerSelfTest()
        {
            ...
            bool accepted = RimMTRuntime.Scheduler.ParallelFor("diagnostics.selfTest", 0, end, 50000,
                ...
                {
                    stopwatch.Stop();
                    Log.Message("[RimMT] Worker self-test passed: workers=" + RimMTRuntime.Scheduler.WorkerCount
                        + ", elapsedMs=" + stopwatch.ElapsedMilliseconds + ", checksum=" + total);
                }, JobPriority.High);
