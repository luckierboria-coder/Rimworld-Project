...
namespace RimMT
{
    // V0.4.19-JS1: keep expensive JobGiver search reuse strictly inside one synchronous
    // JobGiver_Work.TryIssueJobPackage call. Nothing survives the package boundary.
    //
    // Two conservative accelerators live in this scope:
    // 1) memoize pure boolean WorkGiver queries (ShouldSkip / HasJobOnThing / HasJobOnCell)
    //    for the exact same WorkGiver, Pawn and target. First repeat hits and periodic 1/32
    //    hits are re-run through Vanilla; a mismatch permanently disables that exact method.
    // 2) reuse the nearest-first Thing[] produced by V0.4.18.1 when the same source IList,
    //    root and maxDistance are queried again inside the same package. Cheap source probes
    //    guard against obvious in-package mutations. Job creation/reservations are never cached.
    internal static class JobPackageLocalSearch0419
    {
        ...
        internal static bool InPackage
        {
            get { return current != null && packageDepth > 0; }
        }
        ...
        internal static void Apply(Harmony harmony)
        {
            ...
            try
            {
                ...
                if (jobGiver == null)
                {
                    Log.Warning("[RimMT] V0.4.19-JS1 unavailable: JobGiver_Work.TryIssueJobPackage(Pawn, JobIssueParams) was not found.");
                    return;
                }
                ...
                Log.Message("[RimMT] V0.4.19-JS1 JobPackage-local search cache active: scope=" + scopePatched +
                    ", workQueries=" + patchedWorkQueries +
                    ", nearestHooks(global/reachable)=" + globalHookPatched + "/" + reachableHookPatched +
                    ". Cache lifetime is one TryIssueJobPackage call only; JobOnThing/JobOnCell, reservations and Jobs are never cached. Boolean cache hits are parity-sampled and method-fused on mismatch.");
            }
            catch (Exception ex)
            {
                Interlocked.Increment(ref patchFailures);
                FeatureGate.Suppress(FeatureId, "JS1 installation failed: " + ex.GetType().Name);
                Log.Warning("[RimMT] V0.4.19-JS1 installation failed; V0.4.18.2 behavior remains. " + ex.GetType().Name + ": " + ex.Message);
            }
        }
        ...
        private static void PatchWorkQueries(Harmony harmony)
        {
            ...
            for (int i = 0; i < allTypes.Count; i++)
            {
                ...
                for (int m = 0; m < methods.Length; m++)
                {
                    ...
                    QueryKind kind;
                    if (!TryClassifyWorkQuery(method, out kind) || !unique.Add(method))
                        continue;
        ...
        private static void PatchNearestOrderHooks(Harmony harmony)
        {
            ...
            MethodInfo prefixMethod = AccessTools.Method(typeof(JobPackageLocalSearch0419), nameof(OrderPrefix));
            MethodInfo postfixMethod = AccessTools.Method(typeof(JobPackageLocalSearch0419), nameof(OrderPostfix));

            if (global != null)
            {
                try
                {
                    HarmonyMethod prefix = new HarmonyMethod(prefixMethod) { priority = Priority.First + 150 };
                    HarmonyMethod postfix = new HarmonyMethod(postfixMethod) { priority = Priority.Last - 150 };
                ...
                {
                    patchFailures++;
                    Log.Warning("[RimMT] V0.4.19-JS1 could not hook nearest GlobalPrefix: " + ex.GetType().Name + ": " + ex.Message);
                }
            }
            ...
            if (reachable != null)
            {
                try
                {
                    HarmonyMethod prefix = new HarmonyMethod(typeof(JobPackageLocalSearch0419), nameof(OrderReachablePrefix)) { priority = Priority.First + 150 };
                    HarmonyMethod postfix = new HarmonyMethod(typeof(JobPackageLocalSearch0419), nameof(OrderReachablePostfix)) { priority = Priority.Last - 150 };
                ...
                {
                    patchFailures++;
                    Log.Warning("[RimMT] V0.4.19-JS1 could not hook nearest GlobalReachablePrefix: " + ex.GetType().Name + ": " + ex.Message);
                }
            }
        }
        ...
        public static void WorkQueryPostfix(bool __result, WorkQueryState __state)
        {
            ...
            if (__state.Verify && __state.Parity != null)
            {
                ...
                else
                {
                    ...
                    {
                        Log.Warning("[RimMT] V0.4.19-JS1 boolean-query parity mismatch; caching disabled for method " +
                            __state.Key.Method + ". cached=" + __state.Cached + ", live=" + __result +
                            ". Vanilla result remains authoritative for this sampled call and all future calls to that method.");
                    }
                }
                return;
            }
        ...
        // Hook for JobGiverGlobalNearest04181.GlobalPrefix(object[]).
        ...
        // Hook for JobGiverGlobalNearest04181.GlobalReachablePrefix(object[]).
            ...
            ref OrderState state)
        {
            ...
            PackageContext context = current;
            if (context == null || packageDepth <= 0 || args == null ||
                args.Length <= priorityIndex || args[priorityIndex] != null)
                return true;
            ...
            if (context.Ordered.TryGetValue(key, out entry))
            {
                if (ValidateCheapSourceProbe(source, entry))
                {
                    ...
                    state.CacheHit = true;
                    return false; // Skip only RimMT's synchronous reorder; GenClosest still runs Vanilla.
                }
        ...
        {
            if (!state.Store || state.Context == null || !ReferenceEquals(current, state.Context) ||
                args == null || args.Length <= setIndex)
                return;
            ...
            OrderedEntry entry = new OrderedEntry(
                ...
                state.Last,
                ordered);
            state.Context.Ordered[state.Key] = entry;
        ...
        private static bool TryClassifyWorkQuery(MethodInfo method, out QueryKind kind)
        {
            ...
            {
                if ((p.Length == 2 || p.Length == 3) && typeof(Thing).IsAssignableFrom(p[1].ParameterType) &&
            ...
            {
                if ((p.Length == 2 || p.Length == 3) && p[1].ParameterType == typeof(IntVec3) &&
                    (p.Length == 2 || p[2].ParameterType == typeof(bool)))
                {
        ...
        private sealed class MethodParityState
        {
            internal long Hits;
        ...
        private struct WorkQueryKey : IEquatable<WorkQueryKey>
        {
        ...
        private struct OrderKey : IEquatable<OrderKey>
        {
            ...
            public bool Equals(OrderKey other)
            {
                return ReferenceEquals(Source, other.Source) && X == other.X && Z == other.Z &&
                    MaxDistance.Equals(other.MaxDistance) && Reachable == other.Reachable;
            }
