...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    {
        if (!source) { g_nativeStage.store(static_cast<int>(NativeStage::ErrorBadBackbuffer)); return false; }

        ComPtr<ID3D11Device> sourceDevice;
        source->GetDevice(&sourceDevice);
        if (!sourceDevice) { g_nativeStage.store(static_cast<int>(NativeStage::ErrorNoDevice)); return false; }
        ...
        if (!g_device || !g_context) { g_nativeStage.store(static_cast<int>(NativeStage::ErrorNoDevice)); return false; }

        D3D11_TEXTURE2D_DESC desc{};
        source->GetDesc(&desc);
        if (!desc.Width || !desc.Height || desc.SampleDesc.Count != 1) { g_nativeStage.store(static_cast<int>(NativeStage::ErrorBadBackbuffer)); return false; }

        if (g_previousFrame && g_currentFrame && g_generatedFrame && g_frameWidth == static_cast<int>(desc.Width) && g_frameHeight == static_cast<int>(desc.Height) && g_frameFormat == desc.Format)
            return true;
        ...
        if (FAILED(g_device->CreateTexture2D(&history, nullptr, &g_previousFrame)) || FAILED(g_device->CreateTexture2D(&history, nullptr, &g_currentFrame)))
        {
            g_nativeStage.store(static_cast<int>(NativeStage::ErrorHistoryTexture));
            return false;
        }
        ...
        {
            g_nativeStage.store(static_cast<int>(NativeStage::ErrorOutputTexture));
            return false;
        }

        UINT formatSupport = 0;
        const bool formatQueryable = SUCCEEDED(g_device->CheckFormatSupport(desc.Format, &formatSupport));
        const bool canSrv = formatQueryable && (formatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE) != 0;
        const bool canUav = formatQueryable && (formatSupport & D3D11_FORMAT_SUPPORT_TYPED_UNORDERED_ACCESS_VIEW) != 0;
    ...
    {
        if (g_device && g_context) { g_d3d11Ready.store(true, std::memory_order_release); return true; }
    ...
    bool GeneratePredictedFrame(ID3D11Texture2D* source, const MetadataSlot& slot)
    {
        ...
        if (tier != RimFGFlow::QualityTier::Bypass && g_computeReady && g_previousSrv && g_currentSrv && g_generatedUav && g_interpolateCs && g_motionConstants)
        {
            g_gpuBudget.Begin(g_context.Get());
            bool useFlow = false;
            if (tier == RimFGFlow::QualityTier::ResidualFlow && g_flowReady)
...
extern "C" UNITY_INTERFACE_EXPORT void RimFG_SetEnabled(int enabled)
{
    ...
    if (enabled) RimFGPresent::SetBackbufferGenerationCallback(&GenerateFromBackbuffer);
    else RimFGPresent::ClearGeneratedFrameSource();
}
