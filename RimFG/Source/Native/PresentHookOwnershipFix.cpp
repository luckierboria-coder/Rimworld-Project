...
namespace RimFGPresent
{
    namespace
    {
        ...
        std::atomic<std::int64_t> g_lastOutputNs{0};
        ...
        LRESULT CALLBACK DummyWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) { return DefWindowProcW(hwnd, msg, wp, lp); }
        ...
        bool IsCurrentProcessWindow(HWND hwnd)
        {
            ...
            RECT rc{};
            return GetClientRect(hwnd, &rc) && (rc.right - rc.left) >= 320 && (rc.bottom - rc.top) >= 240;
        }

        bool IsSourceWindowUsable(HWND hwnd) { return IsCurrentProcessWindow(hwnd) && IsWindowVisible(hwnd) && !IsIconic(hwnd); }
        ...
        bool IsTargetSwapChain(IDXGISwapChain* swapChain)
        {
            ...
            if (g_unityDevice)
            {
                ...
                if (FAILED(swapChain->GetDevice(__uuidof(ID3D11Device), reinterpret_cast<void**>(device.GetAddressOf()))) || device.Get() != g_unityDevice) return false;
            }
            return true;
        }
        ...
        bool GetCurrentBackBuffer(IDXGISwapChain* chain, ComPtr<ID3D11Texture2D>& out)
        {
            ...
            if (SUCCEEDED(chain->GetBuffer(index, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(out.GetAddressOf()))) && out) return true;
            out.Reset();
        ...
        SharedFrameBatch ReadLatestBatch()
        {
            ...
            return g_batchSlots[g_batchSequence.load(std::memory_order_acquire) & 1u];
        }
        ...
        bool SetCompositionVisible(bool visible)
        {
            ...
            if (FAILED(g_dcompVisual->SetContent(visible ? g_outputSwapChain.Get() : nullptr)) || FAILED(g_dcompDevice->Commit()))
            {
        ...
        bool CreateIndependentDevice(IDXGIAdapter* adapter)
        {
            ...
            HRESULT hr = D3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
                requested, 2, D3D11_SDK_VERSION, &g_outputDevice, &created, &g_outputContext);
            if (hr == E_INVALIDARG)
            {
                ...
                hr = D3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
                    fallback, 1, D3D11_SDK_VERSION, &g_outputDevice, &created, &g_outputContext);
            }
        ...
        {
            if (!batch.sourceWindow || batch.width <= 0 || batch.height <= 0) return false;
            ...
            if (FAILED(unityChain->GetDevice(__uuidof(ID3D11Device), reinterpret_cast<void**>(unityDevice.GetAddressOf()))) || !unityDevice ||
                FAILED(unityDevice.As(&unityDxgi)) || FAILED(unityDxgi->GetAdapter(&adapter)) ||
                FAILED(adapter->GetParent(__uuidof(IDXGIFactory2), reinterpret_cast<void**>(factory.GetAddressOf()))) ||
                !CreateIndependentDevice(adapter.Get())) return false;

            ComPtr<IDXGIDevice> outputDxgi;
            if (FAILED(g_outputDevice.As(&outputDxgi)) ||
                FAILED(DCompositionCreateDevice(outputDxgi.Get(), __uuidof(IDCompositionDevice), reinterpret_cast<void**>(g_dcompDevice.GetAddressOf()))) ||
                FAILED(g_dcompDevice->CreateTargetForHwnd(batch.sourceWindow, TRUE, &g_dcompTarget)) ||
                FAILED(g_dcompDevice->CreateVisual(&g_dcompVisual)))
            {
                ReleaseCompositionResources();
                return false;
            }
        ...
        OpenedSharedFrame* GetOpenedShared(HANDLE handle)
        {
            ...
            for (auto& entry : g_openedCache)
                if (entry.handle == handle && entry.texture && entry.keyed) return &entry;
            OpenedSharedFrame* target = nullptr;
            for (auto& entry : g_openedCache)
                if (!entry.handle) { target = &entry; break; }
            if (!target)
            {
                ClearOpenedCache();
                target = &g_openedCache[0];
            }
            if (FAILED(g_outputDevice->OpenSharedResource(handle, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(target->texture.GetAddressOf()))) || !target->texture) return nullptr;
            if (FAILED(target->texture.As(&target->keyed)) || !target->keyed)
            {
        ...
        bool ReclaimSharedHandle(HANDLE handle, DWORD timeoutMs)
        {
            if (!handle) return true;
            OpenedSharedFrame* shared = GetOpenedShared(handle);
            if (!shared || !shared->keyed) return false;
            const HRESULT hr = shared->keyed->AcquireSync(1, timeoutMs);
            if (hr != S_OK)
            {
                g_frameLatencyTimeoutCount.fetch_add(1, std::memory_order_relaxed);
                return false;
            }
            shared->keyed->ReleaseSync(0);
            return true;
        }

        void ReclaimRemaining(const SharedFrameBatch& batch, int cursor)
        {
            if (batch.batchId == 0) return;
            const int start = std::max(0, cursor);
            const int end = std::max(0, std::min(batch.frameCount, MaxBatchFrames));
            for (int i = start; i < end; ++i)
                ReclaimSharedHandle(batch.handles[i], 50);
        }
        ...
        bool GetCurrentOutputBackBuffer(ComPtr<ID3D11Texture2D>& out)
        {
            ...
            if (SUCCEEDED(g_outputSwapChain->GetBuffer(index, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(out.GetAddressOf()))) && out) return true;
            out.Reset();
        ...
        bool PresentSharedHandle(HANDLE handle)
        {
            OpenedSharedFrame* shared = GetOpenedShared(handle);
            if (!shared || !shared->texture || !shared->keyed) return false;
            const HRESULT acquire = shared->keyed->AcquireSync(1, 10);
            if (acquire != S_OK)
            {
                g_frameLatencyTimeoutCount.fetch_add(1, std::memory_order_relaxed);
                return false;
            ...
            const std::int64_t lastNs = g_lastOutputNs.exchange(nowNs, std::memory_order_acq_rel);
            if (lastNs > 0)
            {
                const double dt = static_cast<double>(nowNs - lastNs) / 1e9;
                if (dt > 0.0005 && dt < 0.5)
                {
        ...
        void WaitUntilDeadline(Clock::time_point deadline)
        {
            ...
            if (!g_pacingTimer) g_pacingTimer = CreateWaitableTimerExW(nullptr, nullptr, CREATE_WAITABLE_TIMER_HIGH_RESOLUTION, TIMER_ALL_ACCESS);
            if (!g_pacingTimer)
            {
            ...
            if (!SetWaitableTimerEx(g_pacingTimer, &due, 0, nullptr, nullptr, nullptr, 0)) std::this_thread::sleep_until(deadline);
            else WaitForSingleObject(g_pacingTimer, 1000);
        }

        void PresenterMain()
        {
            SharedFrameBatch active{};
            int cursor = 0;
            Clock::time_point nextOutput{};
            Clock::time_point nextRefresh{};
            ...
            while (!g_presenterStop.load(std::memory_order_acquire))
            {
                ...
                {
                    if (active.batchId != 0) ReclaimRemaining(active, cursor);
                    active = SharedFrameBatch{};
                    cursor = 0;
                    if (g_dcompVisual) SetCompositionVisible(false);
                    nextOutput = Clock::time_point{};
                    std::unique_lock<std::mutex> lock(g_presenterMutex);
                    g_presenterCv.wait_for(lock, std::chrono::milliseconds(10));
                    continue;
                }
                ...
                if (!EnsureCompositionOutput(active))
                {
                    g_compositionFailureCount.fetch_add(1, std::memory_order_relaxed);
                    ReclaimRemaining(active, cursor);
                    active = SharedFrameBatch{};
                ...
                if (static_cast<double>(target) <= baseFps + 0.5)
                {
                    ReclaimRemaining(active, cursor);
                    active = SharedFrameBatch{};
                    cursor = 0;
                ...
                const Clock::time_point now = Clock::now();
                if (nextRefresh.time_since_epoch().count() == 0 || now >= nextRefresh)
                {
                    g_monitorRefreshHz.store(QueryMonitorRefreshHz(active.sourceWindow), std::memory_order_release);
                    nextRefresh = now + std::chrono::seconds(1);
                }
                const int effective = std::max(1, std::min(target, std::max(24, g_monitorRefreshHz.load(std::memory_order_acquire))));
                const auto period = std::chrono::duration_cast<Clock::duration>(std::chrono::duration<double>(1.0 / static_cast<double>(effective)));
                if (nextOutput.time_since_epoch().count() == 0 || effective != lastEffectiveFps || now > nextOutput + period * 3) nextOutput = now;
                lastEffectiveFps = effective;

                if (cursor >= active.frameCount)
                {
                    std::unique_lock<std::mutex> lock(g_presenterMutex);
                    g_presenterCv.wait_for(lock, std::chrono::milliseconds(2));
                    continue;
                }
                ...
                const bool presented = PresentSharedHandle(active.handles[frameIndex]);
                if (presented)
                {
                    if (frameIndex == active.realFrameIndex) g_realPresentCount.fetch_add(1, std::memory_order_relaxed);
                    else g_generatedPresentCount.fetch_add(1, std::memory_order_relaxed);
        ...
        HRESULT __stdcall HookPresent(IDXGISwapChain* swapChain, UINT syncInterval, UINT flags)
        {
            ...
            if (target && (flags & DXGI_PRESENT_TEST) == 0 && SUCCEEDED(hr))
            {
                ...
                if (mode != PresentMode::Disabled && !producerAdvanced) g_ringBusyDropCount.fetch_add(1, std::memory_order_relaxed);
            }
            return hr;
        }
    ...
    bool Initialize(ID3D11Device* unityDevice)
    {
        ...
        if (MH_EnableHook(presentAddress) != MH_OK) return false;
        ...
        g_lastOutputNs.store(0, std::memory_order_release);
    ...
    void SetPresentMode(PresentMode mode) { if (mode == PresentMode::VSync2x) mode = PresentMode::ImmediateValidation; g_presentMode.store(static_cast<int>(mode), std::memory_order_release); g_presenterCv.notify_all(); }
    ...
    {
        if (batch.batchId == 0 || batch.frameCount <= 0 || batch.frameCount > MaxBatchFrames || batch.width <= 0 || batch.height <= 0 || !batch.sourceWindow) return;
    ...
    {
        const std::int64_t last = g_lastOutputNs.load(std::memory_order_acquire);
        if (last <= 0 || NowNs() - last > 500000000LL) return 0.0;
        const double v = g_outputIntervalSeconds.load(std::memory_order_acquire);
        return v > 0.0005 ? 1.0 / v : 0.0;
    }
