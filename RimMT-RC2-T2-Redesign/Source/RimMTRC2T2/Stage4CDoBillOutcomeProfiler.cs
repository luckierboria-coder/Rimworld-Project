...
namespace RimMTRC2T2
{
    ...
    internal static class Stage4CDoBillOutcomeProfiler
    {
        ...
        private static long failures, packages, lastPackage, calls, nulls, jobs, repeats, repeatNulls, repeatJobs;
        ...
        private static void Install()
        {
            try
            {
                ...
                Log.Message("[RimMT] RC2-T2 Stage 4C DoBill Outcome/Repetition Profiler installed. Tail-only telemetry; JobOnThing always executes normally and no result is cached or modified.");
            }
            catch(Exception ex){Interlocked.Increment(ref failures); Log.Warning("[RimMT] RC2-T2 Stage 4C failed closed: "+ex.GetType().Name+": "+ex.Message);}
        }
        ...
        public static void Prefix(object __instance, object[] __args, out State __state)
        {
            ...
            try
            {
                ...
                Thing thing=null; if(__args!=null) for(int i=0;i<__args.Length;i++){thing=__args[i] as Thing; if(thing!=null) break;} if(thing==null) return;
        ...
        public static void ReportPostfix()
        {
            ...
            Log.Message("[RimMT] RC2-T2 Stage 4C DoBill Outcome report: patched="+patched+", packages="+Interlocked.Read(ref packages)+", calls="+c+", result(null/job)="+Interlocked.Read(ref nulls)+"/"+Interlocked.Read(ref jobs)+", repeatedCalls="+rp+" ("+(c==0?0.0:100.0*rp/c).ToString("F1")+"%), repeatedResult(null/job)="+Interlocked.Read(ref repeatNulls)+"/"+Interlocked.Read(ref repeatJobs)+", ms(total/null/job/repeat/max)="+t.ToString("F2")+"/"+n.ToString("F2")+"/"+j.ToString("F2")+"/"+r.ToString("F2")+"/"+m.ToString("F2")+", billCount(0/1/2/3-4/5+)="+Interlocked.Read(ref b0)+"/"+Interlocked.Read(ref b1)+"/"+Interlocked.Read(ref b2)+"/"+Interlocked.Read(ref b34)+"/"+Interlocked.Read(ref b5)+", entryBucket32-63/64-127/128+="+Interlocked.Read(ref e3263)+"/"+Interlocked.Read(ref e64127)+"/"+Interlocked.Read(ref e128)+", failures="+Interlocked.Read(ref failures)+".");
            for(int i=0;i<Math.Min(6,ts.Count);i++){var kv=ts[i];var s=kv.Value;Log.Message("[RimMT]   Stage4C ThingDef #"+(i+1)+" "+kv.Key.defName+": calls="+s.Calls+", null/job="+s.Nulls+"/"+s.Jobs+", repeats="+s.Repeats+", totalMs="+s.Ms.ToString("F2")+".");}
            for(int i=0;i<Math.Min(6,ws.Count);i++){var kv=ws[i];var s=kv.Value;Log.Message("[RimMT]   Stage4C WorkGiver #"+(i+1)+" "+kv.Key.defName+": calls="+s.Calls+", null/job="+s.Nulls+"/"+s.Jobs+", repeats="+s.Repeats+", totalMs="+s.Ms.ToString("F2")+".");}
        }
    }
}
